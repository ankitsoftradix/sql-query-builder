...
async function handler(req, res) {
  try {
    ...
    if (req.body.aggregation) {
      if (
        req.body.aggregation == "COUNT" ||
        req.body.aggregation == "SUM" ||
        req.body.aggregation == "AVG"
      ) {
    ...
    if (req.body.is_join != 1) {
      if (req.body.is_condition && req.body.is_condition.length > 0) {
        for (let i in req.body.is_condition) {
          query += ` ${req.body.is_condition[i].condition_type} ${req.body.is_condition[i].condition_field} = '${req.body.is_condition[i].condition_value}'`;
        }
      }
    }
    ...
    if (req.body.is_join == 1) {
      ...
      if (
        req.body.join_type == "INNER JOIN" ||
        req.body.join_type == "LEFT JOIN" ||
        req.body.join_type == "RIGHT JOIN" ||
